...
const renderPagination = (renderOptions, isFirstRender) => {
   ...
   container1.innerHTML = `<p class="text-lg-left text-sm-right text-left"><span class="text-primary font-weight-bold">${nbHits}</span> properties found</p>`;
...
const renderBottomPagination = (renderOptions, isFirstRender) => {
    const {
        pages,
        currentRefinement,
        nbPages,
    ...
    const container = document.querySelector("#pagination-bottom");
    widgetParams.container.innerHTML = `
    ${
        !nbHits
            ? ""
   ...
   // const container1 = document.querySelector("#tot_records");
   const container1 = document.querySelector("#tot_records_bottom");

   container1.innerHTML = `<p class="text-lg-left text-sm-right text-left""><span class="text-primary font-weight-bold">${nbHits}</span> properties found</p>`;
  // document.querySelector(".tot_records").innerHTML = `<p class="text-lg-left text-right"><span class="text-primary font-weight-bold">${nbHits}</span> properties found</p>`;
    myFavoritesFindSpace();
};
const customBottomPagination = instantsearch.connectors.connectPagination(
...
search.addWidgets([
    customConfigure({
        container: document.querySelector("#configure-bottom"),
        searchParameters: {
           // hitsPerPage: 30
           hitsPerPage: hitsPerPage
        }
...
search.addWidgets([
    customSortBy({
        ...
        items: [            
            ...
            { label: "Recommended Space", value: indexNameFeatured },           
...
// Instantiate the custom widget
// instantsearch.widgets.index({ indexName: 'space_match_properties_index_featured' }).addWidgets([
//     customSortBy({
//         container: document.querySelector("#sort-by-bottom"), 
//         items: [
//             { label: "Featured", value: indexNameFeatured },
//             { label: "Newest", value: indexName },
//             { label: "Price: Highest to Lowest", value: indexNameDesc },
//             { label: "Price: Lowest to Highest", value: indexNameAsc },
//             { label: "Size: Largest to Smallest", value: indexNameSizeDesc },
...
const renderGeoSearch = (renderOptions, isFirstRendering) => {
    ...
    //         second_array.push({name: items.lat,  index:  items.lng});
    //   });
    ...
    function setMap(){
      //  markers.forEach(marker => marker.remove());
        ...
        //         locations.push({lat: val._geoloc.lat,lng: val._geoloc.lng}); 
        //     }); 
        // }
...
const renderClearRefinements = (renderOptions, isFirstRender) => {
    ...
    if (isFirstRender) {
      ...
      button.setAttribute('data-target','#features-amenities');  
      button.setAttribute('aria-expanded','false');  
      button.setAttribute('aria-controls','features-amenities');  
